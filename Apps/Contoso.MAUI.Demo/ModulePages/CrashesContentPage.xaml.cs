...
using System.IO;
using Contoso.MAUI.Demo.Views;
using BD.AppCenter;
using BD.AppCenter.Crashes;
using System.Linq;
using System.Threading.Tasks;
//using Contoso.MAUI.Demo.UtilClassLibrary;
...
public partial class CrashesContentPage
{
    ...
    protected override async void OnAppearing()
    {
        ...
        // Attachments
        if (Preferences.ContainsKey(TextAttachmentKey))
        {
            TextAttachmentContent.Text = Preferences.Get(TextAttachmentKey, string.Empty);
        }
        if (Preferences.ContainsKey(FileAttachmentKey))
        {
            try
            {
                BinaryAttachmentFilePathLabel.Text = Preferences.Get(FileAttachmentKey, string.Empty);
            }
            catch (Exception e)
            {
                Debug.WriteLine("Couldn't read file attachment: {0}", e.Message);
                Preferences.Remove(FileAttachmentKey);
            }
        }
    }
    ...
    async void TextAttachment(object sender, EventArgs e)
    {
        ...
        Preferences.Set(TextAttachmentKey, text);
    }
    ...
    async void FileAttachment(object sender, EventArgs e)
    {
        ...
        BinaryAttachmentFilePathLabel.Text = fileResult.FullPath;
        Preferences.Set(FileAttachmentKey, fileResult.FullPath);
    }
